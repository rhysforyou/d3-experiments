...
if (getURLParameter("repo")) {
  ...
  startGraph(getURLParameter("repo"));
}
...
$("#repo-form form").submit(function(event) {
  event.preventDefault();
  repoPath = $(event.target).find("input").val();
  $("#repo-form").css("display", "none");
  window.history.pushState('graph', repoPath, "?repo=" + repoPath);
  startGraph(repoPath);
});

function startGraph(rootRepo) {
...
function expandNode(d) {
  if (d.type === "repo") {
    var url = "https://api.github.com/repos/" + d.full_name + "/contributors?anon=1"
  } else if (d.type === "user") {
    var url = "https://api.github.com/users/" + d.login + "/repos"
  } else {
    return;
  }
